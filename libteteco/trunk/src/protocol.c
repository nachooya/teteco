...
protocol_t protocol_parse_datagram (char* datagram, int datagram_len) {
    ...
    if (PRO_HAS_CONTROL(protocol_type)) {
        ...
        memcpy (&protocol.control.seq, &datagram[cursor], PRO_CONTROL_SEQ_SIZE);
        cursor+=PRO_CONTROL_SEQ_SIZE;
...
int protocol_build_datagram (protocol_t protocol, char** datagram, unsigned int *datagram_len) {
    ...
    *datagram_len = PRO_TYPE_SIZE +
                    ...
                    (protocol.control.has ? PRO_CONTROL_TYPE_SIZE + PRO_CONTROL_SEQ_SIZE + PRO_CONTROL_ARG_SIZE + PRO_CONTROL_ARG_SIZE : 0) +
    ...
    *datagram = util_calloc (1, *datagram_len);
    ...
    if (protocol.control.has) {
        ...
        memcpy (*datagram+cursor, &protocol.control.seq, PRO_CONTROL_SEQ_SIZE);
        cursor += PRO_CONTROL_SEQ_SIZE;
...
void protocol_print (protocol_t protocol) {
    ...
    log_print ("protocol.control.seq = 0x%02X\n", protocol.control.seq);
